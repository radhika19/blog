...
def blogs_list(request):
    ...
    if request.method == 'GET':
        ...
        #return Response({'Success':'Text saved', 'code':402})
...
def blog_list(request, pk):
    ...
    if request.method == 'GET':
        ...
        paragraphs = Paragraph.objects.filter(blog=blog)
        response['paragraphs'] = []
	...
	return Response(response)
	    ...
	    data = request.data
            ...
            for comment in comments:
                com_save = Comment(paragraph_id=int(comment.items()[0][0]), comment=comment.items()[0][1])
                com_save.save()
            return Response({"status":"success","message":"Comment successfully added"}, status=status.HTTP_201_CREATED)
        except Exception as ex:
            return Response({"status":"fail",'message':str(ex)}, status=status.HTTP_400_BAD_REQUEST) 
